...
{
	mDefense_names = new Text*[DEFENSETYPES_TOTAL];
	mDefense_values = new Text*[DEFENSETYPES_TOTAL];
	...
	{
		dest.y += 20;
		mDefense_names[i] = new Text(Defense::get_name(DEFENSETYPES(i)), dest, WINDOWCONTENT, text_color, this);
		mDefense_values[i] = new Text(std::to_string(mUnit->get_defense()->get_display_defenses().get_defense(DEFENSETYPES(i))), dest, WINDOWCONTENT, text_color, this);
...
void UnitWindow::render()
{
	...
	for (auto i = 0; i < DEFENSETYPES_TOTAL; i++)
	{
		mDefense_values[i]->set_text(std::to_string(mUnit->get_defense()->get_display_defenses().get_defense(DEFENSETYPES(i))));
	}
